...
With the **[timeline](tools.html#practice) focused** (<kbd>3</kbd>): <kbd>↑</kbd>/<kbd>↓</kbd> select a row, <kbd>Space</kbd> play/pause from any row, <kbd>M</kbd> mutes the selected track, <kbd>Enter</kbd> jumps to the start (or the loop in-point when looped), <kbd>←</kbd>/<kbd>→</kbd> seek by the current step, <kbd>+</kbd>/<kbd>−</kbd> cycle that step through **1 / 5 / 10 / 30&nbsp;s**, <kbd>G</kbd> edits the selected track's gain, <kbd>H</kbd> moves the selected clip, <kbd>E</kbd> [exports](tools.html#export) the unmuted takes, <kbd>[</kbd>/<kbd>]</kbd>/<kbd>L</kbd> set and toggle the loop, and <kbd>Delete</kbd> removes the row.
